/**
 * Creates Supabase Auth test users and re-seeds the entire database using their real UUIDs.
 * Run with: pnpm --filter @avimus/db exec tsx scripts/seed-auth-users.ts
 *
...
const USER_DEFS = [
  { email: 'ricardo@saolucas.com',      name: 'Dr. Ricardo Alves',   role: 'admin',         tenantId: 'tenant-a-uuid-0001', tenantName: 'Clínica São Lucas' },
  { email: 'fernanda@saolucas.com',     name: 'Dra. Fernanda Costa', role: 'medico',        tenantId: 'tenant-a-uuid-0001', tenantName: 'Clínica São Lucas' },
  { email: 'carla@saolucas.com',        name: 'Carla Souza',         role: 'recepcionista', tenantId: 'tenant-a-uuid-0001', tenantName: 'Clínica São Lucas' },
  { email: 'paulo@hospitalvitoria.com', name: 'Dr. Paulo Mendes',    role: 'admin',         tenantId: 'tenant-b-uuid-0001', tenantName: 'Hospital Vitória'  },
  { email: 'ana@hospitalvitoria.com',   name: 'Dra. Ana Lima',       role: 'medico',        tenantId: 'tenant-b-uuid-0001', tenantName: 'Hospital Vitória'  },
  { email: 'joao@hospitalvitoria.com',  name: 'João Oliveira',       role: 'recepcionista', tenantId: 'tenant-b-uuid-0001', tenantName: 'Hospital Vitória'  },
]
...
async function listAllAuthUsers(): Promise<AuthUser[]> {
  ...
  let page = 1
  while (true) {
    const res = await fetch(`${ADMIN}/users?page=${page}&per_page=50`, { headers: H })
    if (!res.ok) throw new Error(`List users failed: ${res.status} ${await res.text()}`)
    const data = await res.json() as { users: AuthUser[]; total?: number }
    all.push(...data.users)
...
async function deleteAuthUser(id: string) {
  const res = await fetch(`${ADMIN}/users/${id}`, { method: 'DELETE', headers: H })
  if (!res.ok && res.status !== 404) throw new Error(`Delete user ${id} failed: ${res.status} ${await res.text()}`)
}

async function createAuthUser(def: typeof USER_DEFS[0]): Promise<string> {
  const res = await fetch(`${ADMIN}/users`, {
    ...
    body: JSON.stringify({
      ...
      password: PASSWORD,
      email_confirm: true,
      user_metadata: { name: def.name, role: def.role, tenant_id: def.tenantId, tenant_name: def.tenantName },
    }),
  })
  if (!res.ok) throw new Error(`Create user ${def.email} failed: ${res.status} ${await res.text()}`)
...
async function main() {
  ...
  try {
    // ── 1. Wipe & recreate Supabase Auth users ────────────────────────────────
    console.log('=== Step 1: Syncing Supabase Auth users ===\n')

    const testEmails = new Set(USER_DEFS.map(u => u.email))
    const existing = await listAllAuthUsers()
    const toDelete = existing.filter(u => testEmails.has(u.email))

    if (toDelete.length > 0) {
      console.log(`Deleting ${toDelete.length} existing auth user(s)...`)
      await Promise.all(toDelete.map(u => deleteAuthUser(u.id)))
    }

    const uuids: Record<string, string> = {}
    for (const def of USER_DEFS) {
      uuids[def.email] = await createAuthUser(def)
      console.log(`  ✓ ${def.email} → ${uuids[def.email]}`)
    }

    const [a1, a2, a3] = [uuids['ricardo@saolucas.com'], uuids['fernanda@saolucas.com'], uuids['carla@saolucas.com']]
    const [b1, b2, b3] = [uuids['paulo@hospitalvitoria.com'], uuids['ana@hospitalvitoria.com'], uuids['joao@hospitalvitoria.com']]

    // ── 2. Re-seed database ───────────────────────────────────────────────────
    console.log('\n=== Step 2: Re-seeding database ===\n')
    ...
    // Tenants
    const tenantA = await prisma.tenant.create({ data: { id: 'tenant-a-uuid-0001', name: 'Clínica São Lucas', slug: 'clinica-sao-lucas', plan: 'professional' } })
    const tenantB = await prisma.tenant.create({ data: { id: 'tenant-b-uuid-0001', name: 'Hospital Vitória', slug: 'hospital-vitoria', plan: 'enterprise' } })

    // Users (with real Supabase UUIDs)
    const usersA = await Promise.all([
      prisma.user.create({ data: { id: a1, tenantId: tenantA.id, email: 'ricardo@saolucas.com',   name: 'Dr. Ricardo Alves',   role: 'admin'         } }),
      prisma.user.create({ data: { id: a2, tenantId: tenantA.id, email: 'fernanda@saolucas.com',  name: 'Dra. Fernanda Costa', role: 'medico'        } }),
      prisma.user.create({ data: { id: a3, tenantId: tenantA.id, email: 'carla@saolucas.com',     name: 'Carla Souza',         role: 'recepcionista' } }),
    ])
    const usersB = await Promise.all([
      prisma.user.create({ data: { id: b1, tenantId: tenantB.id, email: 'paulo@hospitalvitoria.com', name: 'Dr. Paulo Mendes',    role: 'admin'         } }),
      prisma.user.create({ data: { id: b2, tenantId: tenantB.id, email: 'ana@hospitalvitoria.com',   name: 'Dra. Ana Lima',       role: 'medico'        } }),
      prisma.user.create({ data: { id: b3, tenantId: tenantB.id, email: 'joao@hospitalvitoria.com',  name: 'João Oliveira',       role: 'recepcionista' } }),
    ])
    console.log(`Users created: ${usersA.length + usersB.length}`)

    // Protocols Tenant A
    const protoColicaA = await prisma.protocol.create({ data: { id: 'proto-colica-a', tenantId: tenantA.id, name: 'Protocolo de Cólica Renal', description: 'Fluxo completo para investigação e tratamento de cólica renal' } })
    ...
    const protoHipertensaoA = await prisma.protocol.create({ data: { id: 'proto-hiper-a', tenantId: tenantA.id, name: 'Protocolo de Hipertensão Arterial', description: 'Acompanhamento de hipertensão arterial sistêmica' } })
    await Promise.all([
    ...
    // Protocols Tenant B
    const protoColicaB = await prisma.protocol.create({ data: { id: 'proto-colica-b', tenantId: tenantB.id, name: 'Protocolo de Cólica Renal', description: 'Fluxo completo para investigação e tratamento de cólica renal' } })
    await Promise.all([
    ...
    const protoHipertensaoB = await prisma.protocol.create({ data: { id: 'proto-hiper-b', tenantId: tenantB.id, name: 'Protocolo de Hipertensão Arterial', description: 'Acompanhamento de hipertensão arterial sistêmica' } })
    await Promise.all([
    ...
    console.log('Protocols and steps created.')
    ...
    ].map((p, i) => prisma.patient.create({ data: { id: `patient-a${i+1}`, tenantId: tenantA.id, fullName: p.name, cpf: p.cpf, birthDate: new Date(p.birth) } })))
    ...
    ].map((p, i) => prisma.patient.create({ data: { id: `patient-b${i+1}`, tenantId: tenantB.id, fullName: p.name, cpf: p.cpf, birthDate: new Date(p.birth) } })))
    console.log(`Patients created: ${patientsA.length + patientsB.length}`)

    // Snapshot helper
    function snap(protoId: string, protoName: string, steps: { id: string; name: string; type: string; orderIndex: number; prerequisiteStepIds: string[]; branchConditions: unknown; dueDays: number | null }[]) {
      return { id: protoId, name: protoName, steps: steps.map(s => ({ id: s.id, name: s.name, type: s.type, orderIndex: s.orderIndex, prerequisiteStepIds: s.prerequisiteStepIds, branchConditions: s.branchConditions as Record<string, string[]>, dueDays: s.dueDays })) }
    }

    const snapColicaA = snap(protoColicaA.id, protoColicaA.name, colicaStepsA)

    // Journeys Tenant A
    const journeyBase = (id: string, patId: string, protoId: unknown, snapshot: unknown, status: string, creator: string, start: string, completed?: string) =>
      prisma.patientJourney.create({ data: { id, tenantId: tenantA.id, patientId: patId, protocolId: protoId as string, protocolSnapshot: snapshot as object, status, createdById: creator, startedAt: new Date(start), ...(completed ? { completedAt: new Date(completed) } : {}) } })

    const j1 = await journeyBase('journey-a1', patientsA[0].id, protoColicaA.id, snapColicaA, 'ativo', a1, '2026-06-20')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a1-1',  tenantId:tenantA.id, journeyId:j1.id, protocolStepId:'psa-1',  status:'concluido',  executedAt:new Date('2026-06-20'), executedById:a3, result:'Paciente triado com sucesso' } }),
    ...
    const j2 = await journeyBase('journey-a2', patientsA[1].id, protoColicaA.id, snapColicaA, 'ativo', a1, '2026-06-18')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a2-1',  tenantId:tenantA.id, journeyId:j2.id, protocolStepId:'psa-1',  status:'concluido', executedAt:new Date('2026-06-18'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a2-2',  tenantId:tenantA.id, journeyId:j2.id, protocolStepId:'psa-2',  status:'concluido', executedAt:new Date('2026-06-19'), executedById:a2, result:'Creatinina elevada' } }),
    ...
    const j3 = await journeyBase('journey-a3', patientsA[2].id, protoColicaA.id, snapColicaA, 'ativo', a2, '2026-06-10')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a3-1',  tenantId:tenantA.id, journeyId:j3.id, protocolStepId:'psa-1',  status:'concluido',   executedAt:new Date('2026-06-10'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a3-2',  tenantId:tenantA.id, journeyId:j3.id, protocolStepId:'psa-2',  status:'concluido',   executedAt:new Date('2026-06-12'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a3-3',  tenantId:tenantA.id, journeyId:j3.id, protocolStepId:'psa-3',  status:'concluido',   executedAt:new Date('2026-06-14'), executedById:a2, result:'Cálculo de 6mm' } }),
    ...
    const j4 = await journeyBase('journey-a4', patientsA[3].id, protoColicaA.id, snapColicaA, 'ativo', a1, '2026-06-08')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a4-1',  tenantId:tenantA.id, journeyId:j4.id, protocolStepId:'psa-1',  status:'concluido',   executedAt:new Date('2026-06-08'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a4-2',  tenantId:tenantA.id, journeyId:j4.id, protocolStepId:'psa-2',  status:'concluido',   executedAt:new Date('2026-06-09'), executedById:a2 } }),
    ...
    const j5 = await journeyBase('journey-a5', patientsA[4].id, protoColicaA.id, snapColicaA, 'ativo', a2, '2026-06-01')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a5-1',  tenantId:tenantA.id, journeyId:j5.id, protocolStepId:'psa-1',  status:'concluido', executedAt:new Date('2026-06-01'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a5-2',  tenantId:tenantA.id, journeyId:j5.id, protocolStepId:'psa-2',  status:'concluido', executedAt:new Date('2026-06-03'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a5-3',  tenantId:tenantA.id, journeyId:j5.id, protocolStepId:'psa-3',  status:'concluido', executedAt:new Date('2026-06-05'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a5-4',  tenantId:tenantA.id, journeyId:j5.id, protocolStepId:'psa-4',  status:'concluido', executedAt:new Date('2026-06-07'), executedById:a1 } }),
      prisma.journeyStep.create({ data: { id:'js-a5-5',  tenantId:tenantA.id, journeyId:j5.id, protocolStepId:'psa-5',  status:'concluido', executedAt:new Date('2026-06-09'), executedById:a1, result:'cirurgico' } }),
    ...
    const j6 = await journeyBase('journey-a6', patientsA[5].id, protoColicaA.id, snapColicaA, 'ativo', a1, '2026-06-02')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a6-1',  tenantId:tenantA.id, journeyId:j6.id, protocolStepId:'psa-1',  status:'concluido', executedAt:new Date('2026-06-02'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a6-2',  tenantId:tenantA.id, journeyId:j6.id, protocolStepId:'psa-2',  status:'concluido', executedAt:new Date('2026-06-04'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a6-3',  tenantId:tenantA.id, journeyId:j6.id, protocolStepId:'psa-3',  status:'concluido', executedAt:new Date('2026-06-06'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a6-4',  tenantId:tenantA.id, journeyId:j6.id, protocolStepId:'psa-4',  status:'concluido', executedAt:new Date('2026-06-08'), executedById:a1 } }),
      prisma.journeyStep.create({ data: { id:'js-a6-5',  tenantId:tenantA.id, journeyId:j6.id, protocolStepId:'psa-5',  status:'concluido', executedAt:new Date('2026-06-10'), executedById:a1, result:'conservador' } }),
    ...
    const j7 = await journeyBase('journey-a7', patientsA[6].id, protoColicaA.id, snapColicaA, 'concluido', a1, '2026-05-01', '2026-06-15')
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a7-1',  tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-1',  status:'concluido', executedAt:new Date('2026-05-01'), executedById:a3 } }),
      prisma.journeyStep.create({ data: { id:'js-a7-2',  tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-2',  status:'concluido', executedAt:new Date('2026-05-03'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a7-3',  tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-3',  status:'concluido', executedAt:new Date('2026-05-06'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a7-4',  tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-4',  status:'concluido', executedAt:new Date('2026-05-10'), executedById:a1 } }),
      prisma.journeyStep.create({ data: { id:'js-a7-5',  tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-5',  status:'concluido', executedAt:new Date('2026-05-12'), executedById:a1, result:'conservador' } }),
      prisma.journeyStep.create({ data: { id:'js-a7-6a', tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-6a', status:'concluido', executedAt:new Date('2026-05-20'), executedById:a2 } }),
      prisma.journeyStep.create({ data: { id:'js-a7-7a', tenantId:tenantA.id, journeyId:j7.id, protocolStepId:'psa-7a', status:'concluido', executedAt:new Date('2026-06-15'), executedById:a1 } }),
    ...
    const j8 = await prisma.patientJourney.create({ data: { id:'journey-a8', tenantId:tenantA.id, patientId:patientsA[7].id, protocolId:protoColicaA.id, protocolSnapshot:snapColicaA, status:'suspenso', createdById:a1, startedAt:new Date('2026-06-15') } })
    await Promise.all([
      prisma.journeyStep.create({ data: { id:'js-a8-1',  tenantId:tenantA.id, journeyId:j8.id, protocolStepId:'psa-1',  status:'concluido', executedAt:new Date('2026-06-15'), executedById:a3 } }),
    ...
    const snapColicaB = snap(protoColicaB.id, protoColicaB.name, colicaStepsB as Parameters<typeof snap>[2])
    ...
    const bResults = [null,null,null,null,'cirurgico','conservador','conservador',null]
    const bJourneyStatuses = ['ativo','ativo','ativo','ativo','ativo','ativo','concluido','suspenso'] as const

    for (let i = 0; i < patientsB.length; i++) {
      const jId = `journey-b${i+1}`
      const start = `2026-06-${String(1+i*2).padStart(2,'0')}`
      const jb = await prisma.patientJourney.create({ data: { id:jId, tenantId:tenantB.id, patientId:patientsB[i].id, protocolId:protoColicaB.id, protocolSnapshot:snapColicaB, status:bJourneyStatuses[i], createdById:b1, startedAt:new Date(start), ...(bJourneyStatuses[i]==='concluido'?{completedAt:new Date('2026-06-24')}:{}) } })
      await Promise.all(bStepIds.map((psId, si) => prisma.journeyStep.create({ data: { id:`js-b${i+1}-${si+1}`, tenantId:tenantB.id, journeyId:jb.id, protocolStepId:psId, status:bStepStatuses[i][si], executedAt:bStepStatuses[i][si]==='concluido'?new Date(`2026-06-${String(1+i*2+si).padStart(2,'0')}`):null, executedById:bStepStatuses[i][si]==='concluido'?(si%2===0?b1:b2):null, result:psId==='psb-5'&&bStepStatuses[i][si]==='concluido'?bResults[i]:null } })))
    }
    console.log('Journeys and steps created.')

    // ── 3. Summary ─────────────────────────────────────────────────────────────
    console.log('\n=== All done! ===\n')
    console.log('Password for all test users: Test@12345\n')
    console.log('Tenant A — Clínica São Lucas:')
    for (const u of usersA) console.log(`  [${u.role.padEnd(14)}] ${u.email}`)
    console.log('\nTenant B — Hospital Vitória:')
    for (const u of usersB) console.log(`  [${u.role.padEnd(14)}] ${u.email}`)
